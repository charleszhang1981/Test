-- ============================================================
-- Online Multiplayer Tetris (Supabase) - FULL RESET SCHEMA
-- ============================================================
-- Intended use:
-- 1) Run in Supabase SQL Editor
-- 2) This script destroys old related tables and recreates schema
-- 3) Includes RLS, indexes, views, and helper functions
-- ============================================================
...
drop function if exists public.create_match_room(bigint);
drop function if exists public.join_match_room(text);
drop function if exists public.set_match_ready(uuid, boolean);
drop function if exists public.set_disconnect_deadline(uuid, uuid, timestamptz);
drop function if exists public.clear_disconnect_deadline(uuid);
drop function if exists public.heartbeat_match(uuid);
drop function if exists public.check_match_timeout(uuid);
drop function if exists public.finish_match(uuid, uuid, text);
drop function if exists public.submit_single_score(integer, integer);
drop function if exists public.submit_single_ai_result(boolean);
drop function if exists public.mark_profile_registered();

drop table if exists public.match_snapshots cascade;
drop table if exists public.matches cascade;
drop table if exists public.player_stats cascade;
drop table if exists public.profiles cascade;
drop table if exists public.tetris_players cascade;
...
-- Indexes
-- -------
create index idx_profiles_registered on public.profiles (is_anonymous);
create index idx_profiles_display_name on public.profiles (display_name);

create index idx_stats_single_score on public.player_stats (single_high_score desc);
create index idx_stats_single_realm_wins on public.player_stats (single_realm_level desc, single_ai_wins desc);
create index idx_stats_online_rank on public.player_stats (online_points desc, online_wins desc);

create index idx_matches_status on public.matches (status);
create index idx_matches_host on public.matches (host_user_id);
create index idx_matches_guest on public.matches (guest_user_id);
create index idx_matches_room_code on public.matches (room_code);

create index idx_snapshots_match_created on public.match_snapshots (match_id, created_at desc);
...
-- ----------------
-- Bootstrap auth.users -> profiles/stats
-- ----------------
...
create function public.create_match_room(p_seed bigint default null)
...
create function public.join_match_room(p_room_code text)
returns public.matches
language plpgsql
security definer
set search_path = public
...
create function public.set_match_ready(
  p_match_id uuid,
  p_is_ready boolean
)
returns public.matches
...
create function public.set_disconnect_deadline(
  p_match_id uuid,
...
create function public.clear_disconnect_deadline(
  p_match_id uuid
)
returns public.matches
language plpgsql
...
create function public.heartbeat_match(
  p_match_id uuid
)
returns public.matches
language plpgsql
...
create function public.check_match_timeout(
  p_match_id uuid
)
returns public.matches
language plpgsql
...
create function public.submit_single_score(
  p_score integer,
...
create function public.submit_single_ai_result(
  p_is_win boolean
)
returns public.player_stats
language plpgsql
...
create function public.mark_profile_registered()
...
-- ============================================================
-- Notes:
-- - Gameplay sync events should go through Supabase Realtime channels.
-- - Persist sparse snapshots at "10 seconds OR 8 locks" cadence.
-- - Call finish_match(...) once per ended game to settle +3 / +0 points.
-- ============================================================
